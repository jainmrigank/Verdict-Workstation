...
The included `notebooks/gemma_bakeoff_colab.ipynb` runs the primary `Gemma 4 E4B IT` pilot on an A100. Before the two-epoch job, it performs a full-sequence training-step preflight and a 30-step contract qualification over five train and three validation cases. All eight responses must pass compact schema, verdict, fact/rule grounding, and the 3,584-token ceiling after at most one repair. The full run requires that matching qualification report. Smaller E2B and Gemma 3 candidates remain explicit fallback experiments. Training enforces no truncation and records resumable, checksummed run manifests. Model artifacts and generated split files remain untracked.
