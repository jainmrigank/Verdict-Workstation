...
The included `notebooks/gemma_bakeoff_colab.ipynb` prefers `Gemma 4 E4B IT`, then tries `Gemma 4 E2B IT`; both require a BF16-capable accelerator for the complete pilot sequence. On a free T4, the notebook uses `Gemma 3 4B IT` with the identical corpus, RAG payload, loss masking, seed, and promotion gates. `Gemma 4 12B IT` remains an optional stronger-accelerator experiment. Training enforces no truncation and records resumable, checksummed run manifests. Model artifacts and generated split files remain untracked.
